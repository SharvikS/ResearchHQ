...
def _check_required_deps() -> list[CheckResult]:
    ...
    for mod, why in REQUIRED_DEPS:
        try:
            ...
            out.append(
                CheckResult(f"dep:{mod}", False, CRITICAL, f"missing ({type(e).__name__}); {why}")
            )
    return out
...
def _check_optional_deps() -> list[CheckResult]:
    ...
    for mod, why in OPTIONAL_DEPS:
        try:
            ...
            logger.debug("Failed", exc_info=True)
            out.append(CheckResult(f"opt:{mod}", False, WARN, f"not installed; {why} unavailable"))
    return out
...
def _check_provider_keys() -> CheckResult:
    ...
    if has_remote:
        return CheckResult(
            "Provider keys", True, INFO, f"configured: {', '.join(set_keys)} + ollama fallback"
        )
    if has_ollama:
        return CheckResult(
            "Provider keys",
...
def _check_output_folder() -> CheckResult:
    ...
    try:
        ...
        logger.debug("Failed", exc_info=True)
        return CheckResult(
            "Output folder writable", False, CRITICAL, f"cannot write to {folder}: {e}"
        )
...
def _check_history_db() -> CheckResult:
    try:
        ...
        logger.debug("Failed", exc_info=True)
        return CheckResult("History DB", False, WARN, f"cannot initialize: {e}")
...
def _check_router_loadable() -> CheckResult:
    ...
    try:
        ...
        return CheckResult("Router providers", True, INFO, f"chain: {' -> '.join(names)}")
    except Exception as e:
        logger.debug("Failed", exc_info=True)
