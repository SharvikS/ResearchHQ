...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--quick", metavar="QUERY", help="Land on the Research screen and auto-launch this query."
    )
    p.add_argument(
        "--workspace", default="default", help="Active workspace name (default: 'default')."
    )
    p.add_argument(
        "--dashboard",
        action="store_true",
    ...
    p.add_argument(
        "--no-tui", action="store_true", help="Bypass the TUI and use the classic CLI instead."
    )
    return p
...
def main() -> None:
    ...
    try:
        ...
        if "textual" in str(e).lower():
            print(
                "ResearchHQ TUI requires the 'textual' package.\n"
                "Install it with:\n"
                '    pip install -e ".[tui]"\n'
